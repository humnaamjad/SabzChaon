...
| File storage | **Supabase Storage** (hybrid — Firebase Storage NOT used) | For tree update photos. As of Feb 2026, Firebase Storage requires the paid Blaze plan (card on file) even for free-tier usage, so the team uses Supabase's free Storage bucket instead. Firestore + Auth remain on Firebase as normal — only photo file storage is on Supabase. Bucket name: `tree-photos` (public). See §10.1 and §16 for keys/implementation. |
...
### 10.0 Photo Upload (Supabase Storage — do this BEFORE calling the AI function)

Before `analyzeTreePhoto` can run, the photo must be uploaded and turned into a public URL. This step uses **Supabase Storage**, not Firebase Storage (see §7 rationale). Implement this in `lib/storage/uploadTreePhoto.ts`:
...
- The returned `publicUrl` is what gets passed as `photoUrl` into `analyzeTreePhoto` (§10 below) AND saved onto the `TreeUpdate` document in Firestore (per §6 schema) — Firestore still stores the *reference* (URL) to the photo; only the actual file bytes live in Supabase.
- Bucket `tree-photos` must be set to "Public" in the Supabase dashboard so these URLs are directly viewable without extra auth headers.

---

All four Coders must implement the AI analysis behind **one single function**, e.g. `lib/ai/treeHealth.ts`, with this exact signature/contract so swapping providers later only touches one file:
...
# NOTE: Firebase Storage is NOT used (see §7) — no NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET needed.
...
# Supabase (client-side, used ONLY for photo file storage — not the database)
...
- Update submission form (photo upload to Supabase Storage per §10.0 + optional text note) → `/api/trees/[id]/updates`
